...
% NOTE:
    ...
    E.P.sigDots     = 25;	% Number of signal dots in the RDK
    ...
    E.P.frameWidDeg	= .05;	% width of frame of fix box (deg);0 means 1 px
...
%% Hardware parameters: user interface, OpenGL, and OS & MATLAB environment
    ...
    % TODO Automatically detect the computer using its hostname:
        % [~, name] = system('hostname');
        % name = trim(name);
    HW.room = '1424scope'; % Projector ('1424') or stereoscope ('1402') room?
    HW.screenNum = 2; % see Screen('Screens?')
    % For scope in 1424, use 0 for single-screen experiments, 2 for multi-screen dev
    % PTB gives better timing in single-screen mode
    ...
    switch lower(HW.room)
        ...
        case '1424scope'
            switch HW.screenNum
                case 1 % dev
                    HW.monWidth	= 50; %estimate
                    HW.viewDist	= 80;
                    HW.useStereoscope = false;
                    knownRoom = true;
                case [2 0] % scope
                    HW.monWidth = 92;
                    HW.viewDist	= 90+20+17+13;
    ...
    switch lower(HW.room)
        ...
        otherwise
            warning('Parameters:NoColorCalib', ...
                'Using basic default (gamma=2) color calibration data!');
    ...
    % Latency - see reqlatencyclass argument of PsychPortAudio('Open?') 
    HW.portAudioLatencyClass = 2;
